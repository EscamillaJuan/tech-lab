...
/**
 * This class simulates a test suit where we implemented a custom
 * MetadataFetcher. A fake implementation is injected
 */
public class ExpirationCheckerTest {
    public static void main(String[] args) {
        ...
        fakeModifiedTimes.put(expired,
                fakeClock.instant().minus(Duration.ofDays(31)));
        fakeModifiedTimes.put(notExpired,
                fakeClock.instant().minus(Duration.ofDays(27)));

        Injector injector = Guice.createInjector(
                binder -> binder.bind(Clock.class).toInstance(fakeClock),
                binder -> binder.bind(MetadataFetcher.class).toInstance(fakeMetadataFetcher)
        );
